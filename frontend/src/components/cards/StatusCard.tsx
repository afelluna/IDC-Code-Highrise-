...
export function StatusCard({ status }: StatusCardProps) {
  return (
    <Card className="p-1.5 h-full">
      ...
      <div className="flex-1 flex flex-col justify-start gap-3 pt-4">
        {status.map((row, i) => {
          const icon = ROW_ICON[row.label];
          return (
            <div
              key={i}
              className="flex justify-between items-center min-h-6"
            >
              <span className="flex items-center gap-2" style={{ color: 'var(--text-secondary)' }}>
                {icon && <Icon name={icon} size={13} strokeWidth={2.2} style={{ color: 'var(--text-muted)' }} />}
              ...
              <div className="flex items-center gap-1.5">
                <span
                  className="text-[12px] font-bold font-mono uppercase"
