...
export class SocketEventController {
  ...
  firstalarm(@ConnectedSocket() socket: any, @MessageBody() message: any = []) {
    let firstAlarmPath =
      config.UPLOAD_STORAGE_DIR +
      "/" +
      config.LOGS_FIRST_ALARM_DIR +
      "/" +
      message[1];
  ...
  @OnMessage("close_gpio")
  close_gpio(@ConnectedSocket() socket: any, @MessageBody() message: any = []) {
    rpiMod.closeGPIO();
  }
